...
const getPuzzles = async (userId) => {
  const puzzles = await prisma.puzzle.findMany({
    ...
    select: {
      ...
      gridData: true, // Progress hesaplamak için lazım
      gameSessions: {
        where: { userId },
        select: { id: true, completedCells: true, status: true },
  ...
  // Her bulmaca için ilerleme yüzdesini hesapla
  return puzzles.map(p => {
    const totalLetters = Array.isArray(p.gridData) ? p.gridData.filter(c => c.type === 'LETTER').length : 0;
    const session = p.gameSessions[0];
    const completedCount = session ? (Array.isArray(session.completedCells) ? session.completedCells.length : 0) : 0;
    ...
    return {
      ...
      gridData: undefined, // Gereksiz veri yükünü azalt
      progress: totalLetters > 0 ? Math.round((completedCount / totalLetters) * 100) : 0,
...
const startGame = async (userId, { puzzleId, difficulty = 'MEDIUM' }) => {
  ...
  if (puzzleId) {
    ...
    if (!selectedPuzzle) throw Object.assign(new Error('Bulmaca bulunamadı.'), { statusCode: 404 });
  } else {
    // Rastgele bir bulmaca bul
    const puzzles = await prisma.puzzle.findMany({
      where: { isActive: true, difficulty },
    });

    if (!puzzles.length) {
      throw Object.assign(new Error('Bu seviyede henüz çengel bulmaca eklenmemiş.'), { statusCode: 404 });
    }

    selectedPuzzle = puzzles[Math.floor(Math.random() * puzzles.length)];
  }

  // Mevcut devam eden bir oturum var mı kontrol et?
  ...
  if (!isOwned) {
    ...
    const puzzleCost = selectedPuzzle.points || 150; // varsayılan maliyet
  ...
  return {
    ...
    puzzle: {
      ...
      width: selectedPuzzle.width,
      height: selectedPuzzle.height,
      gridData: selectedPuzzle.gridData, // Tüm hücrelerin yapısı
    }
  };
};
